...
class UserCreateCommandTest extends TestCase
{
    ...
    public function testExecuteCreatesUserNonInteractivelyFromStandardInput(): void
    {
        $authModeResolver = $this->createMock(AuthModeResolver::class);
        $authUserService = $this->createMock(AuthUserService::class);
        $actionLogService = $this->createStub(ActionLogService::class);
        ...
        $command = new class ($authModeResolver, $authUserService, $actionLogService) extends UserCreateCommand {
        ...
        $exitCode = $tester->execute(
            ['username' => 'admin', '--password-stdin' => true],
            ['interactive' => false]
        );
    ...
    public function testExecuteReturnsFailureWhenPasswordSourceMissingInNonInteractiveMode(): void
    {
        $authModeResolver = $this->createMock(AuthModeResolver::class);
        $authUserService = $this->createMock(AuthUserService::class);
        $actionLogService = $this->createStub(ActionLogService::class);
        ...
        $this->assertStringContainsString('--password-stdin is required', $tester->getDisplay());
    }
